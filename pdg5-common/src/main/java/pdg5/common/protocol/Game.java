...
public class Game extends Message {
   ...
   public Game(int ID, Date created, Date lastActivity,
      ...
      switch (state) {
         case OUTDATED:
            ...
            break;
         case FINISHED:
            int scoreDiff = Integer.compare(board.getScore(), opponentBoard.getScore());
            if (scoreDiff > 0) {
               result = Result.WIN;
            } else if (scoreDiff < 0) {
               result = Result.LOSE;
   ...
   /**
    * allow to change the current State of the game (IN_PROGRESS, END_MODE, FINISHED,
    * OUTDATED) improve the rapidity of answer of the server
    *
